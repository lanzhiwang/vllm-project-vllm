...
* https://zhuanlan.zhihu.com/p/692202970
...
- 先逐请求调用 `_add_request` 方法将请求添加到 llm_engine(LLMEngine 的实例). 调用 `generate` 的时候可以把多个请求一次性传给 `generate` 方法, 可以提升处理请求的并发量.
...
LLMEngine 接受请求并生成输出. 下面是 LLMEngine docstring 对它功能的概括:
...
>
> 这是 vLLM 引擎的主类. 它负责接收来自客户端的请求, 并利用大语言模型(LLM)生成文本. 该类包含分词器、语言模型(可能分布在多个 GPU 上)以及用于存储中间状态(即 KV 缓存)的 GPU 显存空间. 通过采用迭代级调度和高效的内存管理机制, 该类能够最大化服务吞吐量.
>

一句话概括 LLMEngine 的核心: 以 continuous batching 策略处理请求, 并借助高效的显存管理机制(Paged Attention)最大化吞吐量.
...
![图1: LLMEngine 核心组件](./images/20.png)
...
```python
# https://github.com/vllm-project/vllm/blob/v0.2.7/vllm/worker/worker.py
class Worker:
    """
    每个 worker 关联一个 GPU. worker 负责 KV cache 的管理和在 GPU 上运行模型.
    在分布式推理的情况下(例如 Tensor Parallelism), 每个 worker 包含了模型的一部分.
    """
    ...
    def __init__(
        ...
        ...
        # cache_engine 会在调用 init_cache_engine 时进行初始化
...
```python
# https://github.com/vllm-project/vllm/blob/v0.2.7/vllm/worker/model_runner.py
class ModelRunner:
...
CacheEngine 对 KV cache 的管理是通过提前将一大块空间(显存或内存)划分成多个块(block), 每块有 block_size 个 slot, 每个 slot 存放一个 token 对应的 K V 值.
...
class LLMEngine:
    def _init_cache(self) -> None:
        ...
        # 因为所有 worker 执行的推理都一样, 所以取 worker 中最小的一个
        # 以确保不会在某张卡上出现显存不足的情况
...
class Worker:
    ...
    def profile_num_available_blocks(
        ...
        # 调用 model_runner 的 profile_run 执行一次推理以分析显存的使用情况
...
`profile_num_available_blocks`首先调用 model_runner 的`profile_run`方法执行一次 forward 的过程以分析显存的使用情况, 后调用 CacheEngine 的`get_cache_block_size`方法计算一个块要占用的空间, 最后计算可以开辟的 GPU 块和 CPU 块数量.

```python
# https://github.com/vllm-project/vllm/blob/v0.2.7/vllm/worker/cache_engine.py
class CacheEngine:
    @staticmethod
...
块占用空间的计算公式如下: `block_size * 2 * num_head * head_size * num_layers`, 即每个块可以存放 block_size 个 token 对应的 K V 值, 每个 token 对应的 K V 个数为`2 * num_head * head_size * num_layers`, 所以每个块总共要存放`block_size * 2 * num_head * head_size * num_layers`个值, 每个值占用的空间为 dtype_size 个字节(如果 tensor 的 dtype 为 float16, 则 dtype_size 为 2, dtype 为 float32, 则 dtype_size 为 4).
...
class CacheEngine:
    ...
    CacheEngine 负责 GPU cache 和 CPU cache 的初始化和管理, 同时也提供 KV cache 操作相关的方法
    """
    ...
    def __init__(
        ...
        # 初始化 cuda stream 用于 cache 的相关操作
...
![图 2: KV cache 示意图](./images/21.png)
...
- 初始化 block_manager: BlockSpaceManager 实例
...
```python
# https://github.com/vllm-project/vllm/blob/v0.2.7/vllm/core/scheduler.py
class Scheduler:
...
BlockSpaceManager 用于管理 logical token block 和 physical token block 之间的映射关系, 而前面提到的 CacheEngine 是用于 KV cache 的初始化和管理(真正开辟显存空间和内存空间), 它们的关系如图 3 所示:
...
- watermark_blocks: 用于避免频繁换出操作, 解释见 [https://github.com/vllm-project/vllm/pull/11](https://link.zhihu.com/?target=https%3A//github.com/vllm-project/vllm/pull/11)

- gpu_allocator: BlockAllocator 实例, 用于管理空闲的 physical token block

- cpu_allocator: 同 gpu_allocator, 只不过 device 是 CPU

- block_tables: 维护请求 id 到 physical token block 的映射

BlockSpaceManager 的初始化逻辑如下:

```python
# https://github.com/vllm-project/vllm/blob/v0.2.7/vllm/core/block_manager.py
...
这里只介绍 gpu_allocator, 它是 BlockAllocator 实例, 用于管理空闲的 physical token block, 它的实现如下:
...
# https://github.com/vllm-project/vllm/blob/v0.2.7/vllm/block.py
